########################################################################################################
# RWKV-7 Hybrid-mode inference (GPT prefill + RNN generation)
# Based on RWKV-LM/RWKV-v7/rwkv_v7_demo_fast.py
# Requires CUDA kernel JIT compilation (wkv7s)
########################################################################################################
...
########################################################################################################
# Paths — all absolute, derived from project root
########################################################################################################

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
RWKV_V7_DIR = os.path.join(PROJECT_ROOT, "RWKV-LM", "RWKV-v7")
TOKENIZER_PATH = os.path.join(RWKV_V7_DIR, "rwkv_vocab_v20230424.txt")
LAMBADA_PATH = os.path.join(RWKV_V7_DIR, "misc", "lambada_test.jsonl")
CUDA_DIR = os.path.join(RWKV_V7_DIR, "cuda")

########################################################################################################
# Model config — hybrid script expects MODEL_NAME without .pth (appended in code)
########################################################################################################

args = types.SimpleNamespace()

# 0.1B g1d model
args.MODEL_NAME = os.path.join(PROJECT_ROOT, "Models", "rwkv7-g1", "rwkv7-g1d-0.1b-20260129-ctx8192")
args.n_layer = 12
args.n_embd = 768
...
load(name="wkv7s", sources=[os.path.join(CUDA_DIR, "wkv7s_op.cpp"), os.path.join(CUDA_DIR, "wkv7s.cu")],
